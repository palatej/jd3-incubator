...
public final class LtdSarimaModelSpec implements Validatable<LtdSarimaModelSpec> {

    @Override
    public LtdSarimaModelSpec validate() throws IllegalArgumentException {
        if (validator != null) {
    ...
    public static class Builder implements Validatable.Builder<LtdSarimaModelSpec> {
        ...
        Builder free(LtdSarimaModelSpec ref) {
            if (ref == null) {
        ...
        Builder reset(LtdSarimaModelSpec ref) {
            if (ref == null) {
    ...
    public LtdSarimaModelSpec withPeriod(int period) {
        if (this.period == period) {
            return this;
        }
        if (period == 1) {
            return new LtdSarimaModelSpec(validator, 1, d, 0, phi, EMPTY, theta, EMPTY, dphi, EMPTY, dtheta, EMPTY, dvar);
        } else {
            return new LtdSarimaModelSpec(validator, period, d, bd, phi, bphi, theta, btheta, dphi, dbphi, dtheta, dbtheta, dvar);
        }
    }

    public LtdSarimaModelSpec resetParameters(LtdSarimaModelSpec ref) {
        return toBuilder().reset(ref).build();
    }

    public LtdSarimaModelSpec freeParameters(LtdSarimaModelSpec ref) {
        return toBuilder().free(ref).build();
    }

    public LtdSarimaModelSpec fixParameters() {
    ...
    private static final LtdSarimaModelSpec AIRLINE = new LtdSarimaModelSpec(null, 0, 1, 1,
            EMPTY, EMPTY, Parameter.make(1), Parameter.make(1), EMPTY, EMPTY, Parameter.make(1), Parameter.make(1), Parameter.zero());

    public static LtdSarimaModelSpec airline() {
        return AIRLINE;
    }
}
